...
bool solve(){
    ...
    rep(i,n*n*n){
        ...
        ll pre = *pl.begin();
        ll now = *next(pl.begin());
        if(p[now].count(*p[pre].begin())==0){
            p[now].insert(*p[pre].begin());
        }else{
            p[now].erase(*p[pre].begin());
        }
        p[pre].erase(p[pre].begin());
        if(p[pre].size()==0&&p[now].size()==0){
            pl.erase(pl.begin());
            pl.erase(pl.begin());
        }else if(p[pre].size()==0){
            pl.erase(pl.begin());
        }else if(p[now].size()==0){
            pl.erase(next(pl.begin()));
            pl.push_back(*pl.begin());
            pl.pop_front();
        }else{
            pl.push_back(*pl.begin());
            pl.pop_front();

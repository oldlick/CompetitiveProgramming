...
ll calcArea(VV<ll> &p){
  ...
  deque<int> q;
  V<ll> cnt(3);
  multiset<int> areaYMax;
  multiset<int> areaYMin;
  ll ans=INF;
  rep(i,n){
    q.push_back(p[i][2]);
    areaYMax.insert(-p[i][1]);
    areaYMin.insert(p[i][1]);
    cnt[p[i][2]]++;
    while(q.size()>0 && cnt[q.front()]>1){
      cnt[q.front()]--;
      areaYMax.erase(areaYMax.find(-p[i-q.size()+1][1]));
      areaYMin.erase(areaYMin.find(p[i-q.size()+1][1]));
      q.pop_front();
    }
    //debug(q.size(),cnt);
    if(cnt[0]>0 && cnt[1]>0 && cnt[2]>0){
      //areaXrange: i-q.size()+1 ~ i
      ll xLen=abs(p[i-q.size()+1][0]-p[i][0]);
      ll yLen=abs(*areaYMax.begin()+*areaYMin.begin());
      //debug(*areaYMax.begin(),*areaYMin.begin(),xLen,yLen);
      chmin(ans,max(xLen,yLen)*2);
    }
  }
  return ans;
}
...
int main(){
  ...
  rep(i,n){
    inl(x,y,c);
    p1.push_back({x+y,x-y,c});
    p2.push_back({x-y,x+y,c});
  }
  sort(all(p1));
  sort(all(p2));
  ll ans=INF;
  chmin(ans,calcArea(p1));
  chmin(ans,calcArea(p2));
  print(ans);
  return 0;
}
/*

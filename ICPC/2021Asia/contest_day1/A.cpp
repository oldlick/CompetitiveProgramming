...
int main(){
  init();
  inl(n);
  VV<char> zy(n,V<char>(n,'0')),xz(n,V<char>(n,'0')),yx(n,V<char>(n,'0'));
  input(zy,xz,yx);
  reverse(all(zy));
  reverse(all(xz));
  reverse(all(yx));
  //debug(zy,xz,yx);
  VV<V<int>> block(n,VV<int>(n,V<int>(n,1)));
  rep(x,n)rep(y,n)rep(z,n){
    if(zy[z][y]=='0'||xz[x][z]=='0'||yx[y][x]=='0')block[x][y][z]=0;
    else block[x][y][z]=1;
  }
  //debug(block);
  VV<char> zy_(n,V<char>(n,'0')),xz_(n,V<char>(n,'0')),yx_(n,V<char>(n,'0'));
  rep(x,n)rep(y,n)rep(z,n){
    if(block[x][y][z]==1){
      zy_[z][y]='1';
      xz_[x][z]='1';
  ...
  //debug(zy_,xz_,yx_);
  if(zy==zy_ && xz==xz_ && yx==yx_) print("Yes");
  else print("No");
  return 0;
}
...
int main(){
  ...
  ll ans=0;
  while(1){
    auto x=min({*set2.begin(),*set4.begin(),*set6.begin()});
    if(x.second.size()==2 && ans2.size()>2){
      x=min({*set4.begin(),*set6.begin()});
      if(x.second.size()==4 && ans4.size()>0){
        x=min({*set6.begin()});
        if(x.second.size()==6 && ans6.size()>0)break;
      }
      if(x.second.size()==6 && ans6.size()>0){
        x=min({*set4.begin()});
        if(x.second.size()==4 && ans4.size()>0)break;
      }
    }
    if(x.second.size()==4 && ans4.size()>0){
      x=min({*set2.begin(),*set6.begin()});
      if(x.second.size()==2 && ans2.size()>2){
        x=min({*set6.begin()});
        if(x.second.size()==6 && ans6.size()>0)break;
      }
      if(x.second.size()==6 && ans6.size()>0){
        x=min({*set2.begin()});
        if(x.second.size()==2 && ans2.size()>2)break;
      }
    }
    if(x.second.size()==6 && ans6.size()>0){
      x=min({*set2.begin(),*set4.begin()});
      if(x.second.size()==2 && ans2.size()>2){
        x=min({*set4.begin()});
        if(x.second.size()==4 && ans4.size()>0)break;
      }
    ...
    debug(x,x.second.size(),ans2,ans4,ans6);
    ans+=x.first;
    if(x.second.size()==2){
      ans2.push_back(x.second);
      set2.erase({-cnt2[x.second]*500,x.second});
      cnt2.erase(x.second);
      rep(i,100){
        string tmp=toDig(i,2)+x.second;
        if(cnt4.count(tmp)==1){
          set4.erase({-cnt4[tmp]*4000,tmp});
          cnt4.erase(tmp);
        }
      }
      rep(i,10000){
        string tmp=toDig(i,4)+x.second;
        if(cnt6.count(tmp)==1){
          set6.erase({-cnt6[tmp]*300000,tmp});
          cnt6.erase(tmp);
        }
      }
    }
    if(x.second.size()==4){
      ans4.push_back(x.second);
      set4.erase({-cnt4[x.second]*4000,x.second});
      cnt4.erase(x.second);
      string tmp=x.second.substr(2,2);
      if(cnt2.count(tmp)==1){
        set2.erase({-cnt2[tmp]*500,tmp});
        cnt2.erase(tmp);
      }
      rep(i,100){
        string tmp=toDig(i,2)+x.second;
    ...
    if(x.second.size()==6){
      ans6.push_back(x.second);
      set6.erase({-cnt6[x.second]*300000,x.second});
      cnt6.erase(x.second);
      string tmp=x.second.substr(4,2);
      if(cnt2.count(tmp)==1){
        set2.erase({-cnt2[tmp]*500,tmp});
        cnt2.erase(tmp);
      }
      if(cnt4.count(tmp.substr(2,4))==1){
        set4.erase({-cnt4[tmp]*4000,tmp});
        cnt4.erase(tmp);
      }
    }

...
ld dist3(V<ll> a, V<ll> b){return sqrt(abs(a[0]-b[0])*abs(a[0]-b[0])+abs(a[1]-b[1])*abs(a[1]-b[1])+abs(a[2]-b[2])*abs(a[2]-b[2]));} // ２点間の距離
...
int main(){
  ...
  ld ans=(4*M_PI*r*r*r/3.0)*n;
  rep(i,n){
    ld d=dist3(p[i],p[(i+1)%n]);
    ans-=2/3.0*M_PI*(r-d/2)*(r-d/2)*(2*r+d/2);
  }
  print(ans);
  return 0;
}

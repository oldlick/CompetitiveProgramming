...
ll highest(V<ll> a,V<ll> b){
  return b[2]-abs(b[0]-a[0])-abs(b[1]-a[1]);
}
...
int main(){
  ...
  VV<ll> ans(w,V<ll>(d,-INF));
  rep(i,w)rep(j,d)rep(k,n){
    chmax(ans[i][j],highest({i,j,0},p[k]));
  }
  debug(ans);
  rep(i,n)if(ans[p[i][0]][p[i][1]]!=p[i][2])die("No");
  ll ansSum=0;
  rep(i,w)rep(j,d)ansSum+=ans[i][j];
  print(ansSum);
  return 0;
}
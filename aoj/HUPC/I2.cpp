...
int main(){
	...
	vl a(n);
	in1(a);
	vector<vector<pll>> connect(n);
	FOR(i,0,n){
		for(ll j=(i+1)*2;j<=n;j+=i+1){
			connect[j-1].pb(mp(a[i],i));
			connect[i].pb(mp(a[i],j-1));
		}
	}
	ll min=0;
	REP(i,n/2){
	...
	REP(i,n-1){
		...
		do{
			now=q.top();
			q.pop();
		}while(isconnected[now.se]!=0);
		ans+=now.fi;
		EACH(x,connect[now.se])q.push(x);
		isconnected[now.se]=1;
	}
	out(ans);
	return 0;
}
...
void fctcal(ll a,vector<ll> &fct){
	ll end=sqrt(a);
	FOR(i,1,end+1){
		if(a%i==0){
			fct.pb(a/i);
...
int main(){
	...
	ll cnt=1;
	EACH(x,p)cnt*=x.se+1;
	vector<mint> Factrical(cnt+1);
	Factrical[0]=1;
	REP(i,cnt)Factrical[i+1]=Factrical[i]*(i+1);
	mint ans=0;
	REP(i,fct.size()){
		ll tmp=n/fct[i];
		ll mul=1;
		...
		if(fct[i]<=mul)continue;
		ans+=Factrical[cnt-2-i];
	}
	out(ans);
	return 0;
}
...
int main(){
	...
	while(i<n){
		ll sft=1;
		if(i+sft<n && s[i]==s[i+sft]){
			while(i-(sft-1)>=0 && i+sft<n && s[i-(sft-1)]==s[i+sft])sft++;
			if(i-(sft-1)>=0 || i+sft<n)sft--;
			out(i-(sft-1),i+sft);
			i+=sft;
			ans+=pow(2,cnt);
			cnt++;
		...
		sft=2;
		if(i+sft<n && s[i]==s[i+sft]){
			while(i-(sft-2)>=0 && i+sft<n && s[i-(sft-2)]==s[i+sft])sft++;
			if(i-(sft-2)>=0 || i+sft<n)sft--;
			out(i-(sft-2),i+sft);
			i+=sft;
			ans+=pow(2,cnt);
			cnt++;

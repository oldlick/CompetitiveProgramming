...
int main(){
	...
	vl cnt(4);
	vl dif(4);
	dif[0]=sw+1;dif[2]=W-sw;
	dif[1]=sh+1;dif[3]=H-sh;
	char ch[4]={'L','U','R','D'};
	REP(i,N){
		REP(j,4)if(ch[j]==S[i])cnt[j]++;
		REP(j,4)if(cnt[j]>=dif[j])die("NO");
		REP(j,4)if(ch[j]==T[i])cnt[(j+2)%4]--;
		REP(j,4)if(-cnt[j]>=dif[j])cnt[j]++;
		//outv(cnt);
	}
	out("YES");
	return 0;

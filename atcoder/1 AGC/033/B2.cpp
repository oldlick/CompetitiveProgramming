...
int main(){
	...
	char ch[4]={'L','U','R','D'};
	REREP(i,N){
		REP(j,4)if(ch[j]==T[i])safe[(j+2)%4]--;
		REP(j,4)if(safe[j]<=0)safe[j]=0;
		REP(j,4)if(ch[j]==S[i])safe[j]++;
		if(safe[0]+safe[2]>=W)die("NO");
		if(safe[1]+safe[3]>=H)die("NO");
		//outv(safe);
	}
	vl dis(4);
	dis[0]=sw;dis[2]=W-sw-1;
	dis[1]=sh;dis[3]=H-sh-1;
	REP(i,4){
		if(dis[i]<safe[i])die("NO");
	}
	out("YES");
	return 0;
}
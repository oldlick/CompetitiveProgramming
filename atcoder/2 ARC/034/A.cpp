...
#define in3(s,t,u) REP(i,size(s)){in(s[i] , t[i] , u[i]);}
#define in4(s,t,u,v) REP(i,size(s)){in(s[i] , t[i] , u[i] , v[i]);}
#define in5(s,t,u,v,w) REP(i,size(s)){in(s[i] , t[i] , u[i] , v[i] , w[i]);}
...
int main(){
    ...
    REP(i,n){
        inl(a,b,c,d,e);
        chmax(max,a+b+c+d+(ld)e*110/900);
    }
    cout<<max<<endl;
	return 0;
}
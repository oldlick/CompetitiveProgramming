...
int main(){
    ...
    rep(i,n){
        if('0'<=s[0][i] && s[0][i]<='9'){
            if('A'<=s[1][i] && s[1][i]<='Z')uf.unite(s[1][i]-'A',26);
        }else{
            if('0'<=s[1][i] && s[1][i]<='9')uf.unite(s[0][i]-'A',26);
            else uf.unite(s[0][i]-'A',s[1][i]-'A');
        }
    }
    ll ans=1;
    rep(i,n){
        if('0'<=s[0][i] && s[0][i]<='9')continue;
        if(uf.same(s[0][i]-'A',26)==false){
            if(i==0)ans*=9;
            else ans*=10;
            uf.unite(s[0][i]-'A',26);
        }

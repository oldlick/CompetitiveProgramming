...
int main(){
    ...
    inl(n);
    V<ll> t(n),h(n);
    rep(i,n) input(t[i],h[i]);
    //debug(t);
    //debug(h);
    VV<ll> dp(x+1,V<ll>(z+1,-INF));
    dp[0][0]=0;
    rep(i,n){
        for(ll j=x;j>=0;j--)for(ll k=z;k>=0;k--){
            ll j_=j+1;
            ll k_=k+t[i];
            if(j_<=x && k_<=z) chmax(dp[j_][k_],dp[j][k]+h[i]);
        }
        //debug(dp);
    }
    ll ans=0;
    each(x,dp)each(y,x) chmax(ans,y);
    cout<<ans<<endl;
    return 0;
}

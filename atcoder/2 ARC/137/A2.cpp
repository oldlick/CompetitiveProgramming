...
int main(){
    ...
    inl(l,r);
    ll ans=1;
    if(r-l>0 && gcd(l,r)==1)chmax(ans,r-l);
    debug(ans);
    if(r-(l+1)>0 && gcd(l+1,r)==1)chmax(ans,r-(l+1));
    debug(ans);
    if((r-1)-l>0 && gcd(l,r-1)==1)chmax(ans,(r-1)-l);
    debug(ans);
    if((r-1)-(l+1)>0 && gcd(l+1,r-1)==1)chmax(ans,(r-1)-(l+1));
    print(ans);
    return 0;
}
...
ll cnt=0;
double dp(V<ll> c){
    static VV<V<double>> memo(310,VV<double>(310,V<double>(310,-1)));
    if(c==V<ll>{n,0,0,0}) return memo[c[1]][c[2]][c[3]]=0;
    if(memo[c[1]][c[2]][c[3]]!=-1) return memo[c[1]][c[2]][c[3]];
    cnt++;
    //debug(c,cnt);
    V<double> p(c.size());
    rep(i,c.size())p[i]=(double)c[i]/n;
    double ans=0;
    for(int i=0;i<3;i++){
        if(c[i+1]==0)continue;
        c[i]++;

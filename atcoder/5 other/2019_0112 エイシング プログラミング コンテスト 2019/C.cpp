...
int main(){
	...
	REP(i,h)REP(j,w)REP(k,4){
		ll _i=i+dx[k],_j=j+dy[k];
		if(_i<0||_j<0||_i>=h||_j>=w)continue;
		if(s[i][j]!=s[_i][_j])g.unite(i*w+j,_i*w+_j);
	}
	vl cnt(h*w,0);
	REP(i,h)REP(j,w){
		if(s[i][j]!='.')cnt[g.root(i*w+j)]++;
	}
	ll ans=0;
	REP(i,h)REP(j,w){
		ans+=cnt[i*w+j]*(g.size(i*w+j)-cnt[i*w+j]);
	}
	out(ans);
	return 0;
}
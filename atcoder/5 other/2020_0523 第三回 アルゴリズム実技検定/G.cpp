...
int main(){
    ...
    inl(n,x,y);
    x+=300,y+=300;
    pair<ll,ll> goal=pair<ll,ll>(x,y);
    vvl g(600,vl(600));
    REP(i,n){
        inl(a,b);
        a+=300;b+=300;
    ...
    tmp.pb(pair<ll,ll>(300,300));
    g[300][300]=1;
    while(tmp.size()!=0){
        ll curx=tmp.front().fi,cury=tmp.front().se;
        //cout<<g[curx][cury]<<endl;
        if(tmp.front()==goal){
            cout<<g[curx][cury]-1<<endl;
            return 0;
        }
        tmp.pop_front();
        if(g[curx+1][cury+1]==0){
            g[curx+1][cury+1]=g[curx][cury]+1;
            tmp.pb(pair<ll,ll>(curx+1,cury+1));
        }
        if(g[curx+0][cury+1]==0){
            g[curx+0][cury+1]=g[curx][cury]+1;
            tmp.pb(pair<ll,ll>(curx+0,cury+1));
        }
        if(g[curx-1][cury+1]==0){
            g[curx-1][cury+1]=g[curx][cury]+1;
            tmp.pb(pair<ll,ll>(curx-1,cury+1));
        }
        if(g[curx+1][cury+0]==0){
            g[curx+1][cury+0]=g[curx][cury]+1;
            tmp.pb(pair<ll,ll>(curx+1,cury+0));
        }
        if(g[curx-1][cury+0]==0){
            g[curx-1][cury+0]=g[curx][cury]+1;
            tmp.pb(pair<ll,ll>(curx-1,cury+0));

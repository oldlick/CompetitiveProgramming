...
int main(){
    ...
    REP(i,l+1){
        ll addtime=0;
        if(x[i]==true)addtime=t[2];
        if(i-1>=0){
            chmin(dp[i],dp[i-1]+run[0]+addtime);
        }
        if(i-2>=0){
            chmin(dp[i],dp[i-2]+run[1]+addtime);
        }
        if(i-4>=0){
            chmin(dp[i],dp[i-4]+run[2]+addtime);
        }
    }
    FOR(i,1,4){
        chmin(dp[l],dp[l-i]+(t[0]+t[1])/2+t[1]*(i-1));
    }
    cout<<dp[l]<<endl;
	return 0;
}
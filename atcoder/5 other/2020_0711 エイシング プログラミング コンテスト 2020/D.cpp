...
int main(){
	...
	vl am_p(n);am_p[n-1]=1;
	vl am_m(n);am_m[n-1]=1;
	REREP(i,n-1){
		if(cnt-1!=0)am_m[i]=(am_m[i+1]*2)%(cnt-1);
		am_p[i]=(am_p[i+1]*2)%(cnt+1);
	}
	//outv(am_p);outv(am_m);
	ll tmp_cnt=cnt;
	ll num_p=0,num_m=0;
	num_p=Div(m,cnt+1);
	if(cnt-1!=0)num_m=Div(m,cnt-1);
	REP(i,n){
		cnt=tmp_cnt;
		ll cur=0;
		ll ans=1;

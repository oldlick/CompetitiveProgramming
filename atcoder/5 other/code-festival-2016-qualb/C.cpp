...
int main(){
    ...
    V<ll> sum_q(h+1);
    reps(i,1,h+1)sum_q[i]=sum_q[i-1]+q[i-1];
    debug(sum_q);
    ...
    rep(i,w){
        while(true){
            if(max_idx==h)break;
            if(reduce[max_idx]+p[i]*max_idx>reduce[max_idx+1]+p[i]*(max_idx+1))break;
            max_idx++;
        }
        debug(i,max_idx,reduce[max_idx]+p[i]*max_idx);
        ans_minus+=reduce[max_idx]+p[i]*max_idx;
    }
    print(ans-ans_minus);
    return 0;
}

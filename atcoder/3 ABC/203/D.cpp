...
int main(){
  ...
  VV<ll> a(n,V<ll>(n));
  rep(i,n)in1(a[i]);
  ll need=k*k-(k*k/2+1)+1;
  auto isOk =[&](ll key){
    VV<ll> cnt(n+1,V<ll>(n+1));
    rep(i,n)rep(j,n){
      if(i==0||j==0)cnt[i][j]=0;
      cnt[i+1][j+1]+=cnt[i+1][j]+cnt[i][j+1]-cnt[i][j]+(a[i][j]<=key);
    }
    rep(i,n)rep(j,n){
      if(i-k+1<0||j-k+1<0)continue;
      ll tmp=cnt[i+1][j+1]-cnt[i-k+1][j+1]-cnt[i+1][j-k+1]+cnt[i-k+1][j-k+1];

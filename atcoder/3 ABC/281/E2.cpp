...
int main(){
    ...
    reps(i,m-1,n){
        ...
        //debug("2");
        debug(*s1.rbegin(),*s2.begin());
        if(s2.size()!=0 && *s1.rbegin()>*s2.begin()){
            auto tmp1=*s1.rbegin();
            auto tmp2=*s2.begin();
            ans-=tmp1.first;
            ans+=tmp2.first;
        ...
        //debug("3");
        print(ans);
        if(s1.count({a[i-m+1],i-m+1})==1){
            s1.erase({a[i-m+1],i-m+1});
            ans-=a[i-m+1];
        }
        if(s2.count({a[i-m+1],i-m+1})==1){
            s2.erase({a[i-m+1],i-m+1});
        }
        //debug("4");
    }
    return 0;

...
int main(){
    ...
    rep(i,m){
        inl(a,b,c);a--;b--;
        if(c==0){
            g[a+n].push_back(make_pair(b+n,1));
            g[b+n].push_back(make_pair(a+n,1));
        }else{
    ...
    rep(i,k){
        inl(x);x--;
        g[x].push_back(make_pair(x+n,0));
        g[x+n].push_back(make_pair(x,0));
    }
    auto [cost,pre] = dijk(g,0);
    ll ans=min(cost[n-1],cost[n+n-1]);
    if(ans==INF)die(-1);
    print(ans);
    return 0;
}
...
ll bfs(vs &s,ll sh,ll sw){
	ll ans=0,h=s.size(),w=s[0].size();
	vvl cnt(h,vl(w,-1));
	cnt[sh][sw]=0;
	...
	while(q.size()!=0){
		...
		q.pop();
		REP(i,4){
			ll nx=dx[i]+v.fi,ny=dy[i]+v.se;
			if(nx<0 || ny<0 || nx>=h || ny>=w)continue;
			if(s[nx][ny]=='.' && cnt[nx][ny]==-1){
				q.push(mp(nx,ny));
				cnt[nx][ny]=cnt[v.fi][v.se]+1;
				if(ans<cnt[nx][ny])ans=cnt[nx][ny];
			}
		}
	}
	return ans;

...
bool ch(pair<ll,ll> p){
  if(p.fi<0 || p.se<0)return false;
  if(p.fi>=n || p.se>=n)return false;
  return s[p.fi][p.se]=='.';
}

VV<ll> memo;
void dfs(pair<ll,ll> now,ll paintNum){
  ll dx[4]={ 1, 1,-1,-1};
  ll dy[4]={-1, 1, 1,-1};
  rep(i,4){
    debug(now,i);
    ...
    while(1){
      debug(next);
      memo[next.fi][next.se]=paintNum;
      next.fi+=dx[i];
      next.se+=dy[i];
      if(ch(next)==false)break;
      if(memo[next.fi][next.se]!=INF)break;
    }
    debug("ok",next);
    next.fi-=dx[i];
    next.se-=dy[i];
    if(ch(next)==true && memo[next.fi][next.se]==INF)dfs(next,paintNum+1);
  }
  debug(memo);
}
...
int main(){
  init();
  input(n,a,b);a.fi--;a.se--;b.fi--;b.se--;
  s.resize(n);
  input(s);
  memo.resize(n);
  rep(i,n)memo[i].resize(n,INF);
  dfs(a,0);
  return 0;
}
...
ll ans(ll now,ll pre,ll k,VV<V<ll>> &step){
    debug(now,k);
    if(k==0) return now;
    int level=-1;
    ll k_=k;
    ...
    rep(i,level)k__*=2;
    debug(level);
    each(x,step[now][level]){
        bool f=false;
        each(y,step[x][level])if(y==pre)f=true;
        if(f)continue;
        int ret = ans(x,now,k-k__,step);
        if(ret!=-1) return ret;
    }
    return -1;
}
...
int main(){
    ...
    inl(n);
    g.resize(n);
    VV<V<ll>> step(n,VV<ll>(20));
    vector<set<ll>> done(n);
    rep(i,n)done[i].insert(i);
    rep(i,n-1){
        inl(a,b);a--;b--;
        g[a].push_back(b);
        g[b].push_back(a);
        step[a][0].push_back(b);
        step[b][0].push_back(a);
        done[a].insert(b);
        done[b].insert(a);
    }
    rep(level,n){
        ...
        rep(i,n){
            each(x,step[i][level]){
                each(y,step[x][level]){
                    if(done[i].count(y))continue;
                    step[i][level+1].push_back(y);
                    done[i].insert(y);
                    cnt++;
                }
            }

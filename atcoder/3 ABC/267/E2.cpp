...
int main(){
    ...
    cost.resize(n);
    rep(i,n) each(x,g[i])cost[x]+=a[i];

    auto fx = [](pair<ll,ll> x1, pair<ll,ll> x2) -> pair<ll,ll> {
        if(x1.fi<x2.fi) return x1;
        else return x2;
    };
    pair<ll,ll> ex = {numeric_limits<ll>::max(),-1};
    SegTree<pair<ll,ll>> rmq(n, fx, ex);
    rep(i,n)rmq.set(i,{cost[i],i});
    rmq.build();

    ll ans=-INF;

    rep(i,n){
        auto [cost,idx] = rmq.query(0,n);
        debug(cost,idx);
        rmq.update(idx,{INF,idx});
        chmax(ans,cost);
        each(x,g[idx]){
            auto [cost_x,idx_x] = rmq.query(x,x+1);
            rmq.update(x,{cost_x-a[idx],idx_x});
        }
    }
    print(ans);

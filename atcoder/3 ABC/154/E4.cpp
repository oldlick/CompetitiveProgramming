...
ll ans=0;
void dfs(vl &n,ll k,ll cur,ll use,ll cnt,ll under){
	if(cur==n.size()){
		if(use==k){
			ans+=cnt;
			//out(cnt);
	...
	if(under){
		dfs(n,k,cur+1,use,cnt,1);
		dfs(n,k,cur+1,use+1,cnt*9,1);
	}else{
		if(n[cur]==0){
			dfs(n,k,cur+1,use,cnt,0);
		}else{
			dfs(n,k,cur+1,use,cnt,1);
			dfs(n,k,cur+1,use+1,cnt,0);
			dfs(n,k,cur+1,use+1,cnt*n[cur]-1,1);
		}
	}
}
int main(){

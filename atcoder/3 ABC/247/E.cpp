...
ll hoge(V<ll> a){
  ...
  ll n=a.size();
  for(int i=n-1;i>=0;i--){
    if(a[i]==x)maxIdx.push_back(i);
    if(a[i]==y)minIdx.push_back(i);
  }
  //debug(maxIdx,minIdx);
  ll ans=0;
  rep(i,n){
    while(true){
      if(maxIdx.size()==0 || minIdx.size()==0)break;
      if(i<=min(maxIdx.back(),minIdx.back()))break;

      if(min(maxIdx.back(),minIdx.back())==maxIdx.back())maxIdx.pop_back();
      else minIdx.pop_back();
    }
    if(maxIdx.size()==0 || minIdx.size()==0)break;
    ans+=n-max(maxIdx.back(),minIdx.back());
    //debug(maxIdx,minIdx,ans);
  }
  //debug(a,ans);
  return ans;
}
...
int main(){
  ...
  ll ans=0;
  rep(i,n){
    if(y<=a[i] && a[i]<=x)d.push_back(a[i]);
    else{
      ans+=hoge(d);
      d.clear();
    }

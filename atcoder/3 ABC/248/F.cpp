...
int main(){
  ...
  inl(n,p);
  mod=p;
  VV<mint> dp(2,V<mint>(3*n));
  dp[0][1]=1;
  dp[1][0]=1;

  rep(i,n-1){
    VV<mint> dp2(2,V<mint>(3*n));
    rep(j,n){
      if(j+0<3*n) dp2[1][j+0]+=dp[1][j];
      if(j+0<3*n) dp2[1][j+0]+=dp[0][j];
      
      if(j+1<3*n) dp2[1][j+1]+=dp[1][j];
      if(j+1<3*n) dp2[0][j+1]+=dp[0][j];
      
      if(j+1<3*n) dp2[1][j+1]+=dp[1][j];
      
      if(j+1<3*n) dp2[1][j+1]+=dp[1][j];

      if(j+2<3*n) dp2[0][j+2]+=dp[1][j];

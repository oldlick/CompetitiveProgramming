...
int main(){
	...
	REREP(i,K){
		if(inP==INF && a[i]>0)inP=a[i];
		if(inM==INF && a[i]<0)inM=a[i];
	}
	FOR(i,K,N){
		if(outP==INF && a[i]>=0)outP=a[i];
		if(outM==INF && a[i]<0)outM=a[i];
	}
	if(inP!=INF && outP!=INF && inM!=INF && outM!=INF){
		// outM/inP or outP/inM
		// outM*inM or outP*inP
		if(outM*inM<outP*inP) Max*=(mint)outP/inM;
		else Max*=(mint)outM/inP;
		die(Max);
	}
	if(inP!=INF && outM!=INF){
		Max*=(mint)outM/inP;

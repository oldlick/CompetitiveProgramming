...
int main(){
    ...
    debug(n,m,a);
    VV<ll> b(n,V<ll>(n));
    rep(i,n)rep(j,n)b[i][j]=(pow(a[i],a[j],m)+pow(a[j],a[i],m))%m;
    debug(b);
    ll ans=0;
    rep(loop,n-1){
        ll idx1,idx2;
        pair<ll,ll> min_value={INF,0};
        rep(i,a.size()){
            ll idx;
            pair<ll,ll> max_value={-INF,0};
            rep(j,a.size()){
                if(i==j)continue;
                if(max_value.first<b[i][j]){
                    idx=j;
                    max_value={b[i][j],1};
                }else if(max_value.first==b[i][j]){
                    max_value.second++;
                }
            }
        ...
        debug(idx1,idx2,min_value);
        ans+=min_value.first;
        swap(a[idx1],a.back());
        a.pop_back();
        swap(b[idx1],b[a.size()]);
        rep(i,n)swap(b[i][idx1],b[i][a.size()]);
        debug(b);
    }
    print(ans);
    return 0;

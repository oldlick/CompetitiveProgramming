...
pair<ll,ll> range(ll level,ll side,ll l,ll r){
    //debug(level,side,l,r);
    ll border=pow(2,12-level-1);
    if(l<border && r<border) return range(level+1,side*2,l,r);
    if(l>border && r>border) return range(level+1,side*2+1,l-border,r-border);
    ll base=4096*level+pow(2,12-level)*side;
    return {base+l,base+r};
}

pair<ll,ll> make(ll id,ll n){
    ll level=id/4096;
    id -= 4096*level;
    ll side=id/pow(2,12-level);
    id -= pow(2,12-level)*side;
    ll border=pow(2,12-level-1);
    ll l,r;
    if(id<border){
        l=id;
    ...
    l+=pow(2,12-level)*side;
    r+=pow(2,12-level)*side;
    return {min(l,n-1),min(r,n-1)};
}
...
int main(){
    ...
    for(ll i=0;i<q;i++){
        ll l,r;
        cin>>l>>r;l--;r--;
        auto tmp=range(0,0,l,r);
        debug(tmp,make(tmp.first,n),make(tmp.second,n));
        tmp.first++;
        tmp.second++;
        print(tmp);
        cout.flush();

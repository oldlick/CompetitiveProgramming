...
// Shortest Hamilton trail.
bool bit_dp(VV<ll> g){
    ll n=g.size();
    VV<pair<ll,ll>> dp(1<<n,V<pair<ll,ll>>(n,{true,-1}));
    // dp[S][end] = {val,start} (S={Path}. S includes start and end.)
    rep(i,n) dp[1<<i][i] = {true,i};
    reps(S,1,1<<n){
        rep(a,n)rep(b,n){
            if( a==b ) continue;
            if( !(S&(1<<a)) ) continue; // aは含む
            if( S&(1<<b) ) continue; // bは含まない
            if( g[b][a]==0 ) continue;
            auto from = dp[S][a];
            auto pred = from;
            auto to = dp[S|(1<<b)][b];
    ...
    rep(i,n){
        ...
        reps(S,1,1<<n){
            auto tmp=dp[S][i];
            if(dp[S][i].second==-1)continue;
            bool f=true;
            rep(j,n){
                if(g[tmp.second][j]==1 && !(S&(1<<j)))f=false;
            }
            if(f==false)continue;
            if(tmp.first==false)win=false;
        }
        if(win==true)return true;
    }
    return false;
    reps(S,1,1<<n)rep(i,n){
        auto tmp=dp[S][i];
        if(tmp.second==-1)continue;
        bool f=true;
        rep(j,n){
            if(g[j][tmp.second]==1 && !(S&(1<<j)))f=false;
        }
        if(f==false)continue;
        if(tmp.first==true){
            debug(S,i,tmp);

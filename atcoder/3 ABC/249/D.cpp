...
int main(){
  ...
  each(x,b){
    each(y,b){
      if(x.first*y.first>=max_n)break;
      dp[x.first*y.first]+=x.second*y.second;
    }
  }
  //rep(i,dp.size())if(dp[i]!=0)debug(i,dp[i]);
  ll ans=0;
  rep(i,a.size())ans+=dp[a[i]];
  //debug(ans);

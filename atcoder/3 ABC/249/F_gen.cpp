...
int main(){
  ...
  std::uniform_int_distribution<> rand15(0, 15);        // [0, 99] 範囲の一様乱数
  
  ll n=rand15(mt);
  std::uniform_int_distribution<> rand_n(0, n);        // [0, 99] 範囲の一様乱数
  ll k=rand_n(mt);
  print(n,k);
  std::uniform_int_distribution<> rand_2(1, 2);        // [0, 99] 範囲の一様乱数
  std::uniform_int_distribution<> rand_109(-100, 100);        // [0, 99] 範囲の一様乱数
  rep(i,n){
    print(rand_2(mt),rand_109(mt));
  }
  return 0;
}
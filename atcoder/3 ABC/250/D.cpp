...
V<ll> hoge(){
  ...
  V<ll> is_q(n,1);
  for(ll i=2;i*i<=n;i++){
    if(is_q[i]==0)continue;
    for(ll j=i*2;j<=n;j+=i)is_q[j]=0;
  }
  V<ll> q_list;
  reps(i,2,n)if(is_q[i]==1)q_list.push_back(i);
  return q_list;
}
...
int main(){
  ...
  rep(i,q_list.size()){
    rep(j,i){
      ll tmp=q_list[i]*q_list[i]*q_list[i];
      //if(tmp<=n/q_list[j])ans.insert(tmp*q_list[j]);
      if(tmp<=n/q_list[j])ans++;
      else break;
    }
  }
  //debug(ans);

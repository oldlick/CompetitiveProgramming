...
int main(){
  ...
  rep(i,q){
    inl(x_num);x_num--;
    auto x_idx=num_idx[x_num];
    auto right_idx=x_idx;
    if(right_idx!=n-1)right_idx++;
    else right_idx--;
    auto right_num=idx_num[right_idx];
    num_idx[x_num]=right_idx;
    num_idx[right_num]=x_idx;
    idx_num[x_idx]=right_num;
    idx_num[right_idx]=x_num;
    //print(idx_num);
  }
  rep(i,n)idx_num[i]++;
  print(idx_num);

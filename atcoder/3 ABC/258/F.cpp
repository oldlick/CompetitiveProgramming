...
template<>
void debug_sub(const long long &t){
...
void solve(){
    ...
    rep(i,4){
        ...
        g_[i%2]=g_[i%2]/b*b;
        g2.push_back({g_,dist_1(g_,g)*k});
    }
    //debug(g2);
    ll ans=dist_1(s,g)*k;
    rep(i,4)rep(j,4){
        ll tmp=dist_1(s2[i].fi,g2[j].fi);
        tmp+=s2[i].se+g2[j].se;
        for(int l=0;l<2;l++){
            if(s2[i].fi[l]/b==g2[j].fi[l]/b && s2[i].fi[(l+1)%2]/b!=g2[j].fi[(l+1)%2]/b){
                ll add=min({s2[i].fi[l]%b,g2[j].fi[l]%b,b-s2[i].fi[l]%b,b-g2[j].fi[l]%b})*2;
                tmp+=add;
                debug(i,j,l,add);
            }
        }
...
2929499539528677
1751667982985758
[482031503 284479575]:1177830931758226
[964063006 427232765]:1751667982985758
2929499539528677

...
int main(){
    ...
    ll base_cnt=x/w_sum*n;
    x%=w_sum;
    debug(w_sum,base_cnt,x);

    V<ll> sum(n*2+1);
    rep(i,n*2) sum[i+1]=sum[i]+w[i%n];
    debug(sum);
    ...
    rep(i,n*n+10){
        ...
        auto it=lower_bound(all(sum),sum[now]+x);
        debug(i,now,sum[now]+x);
        ll next=it-sum.begin();
        data.push_back(next-now+base_cnt);
        done[now]=i;
        now=next%n;
    }
    debug(data);

...
int main(){
  ...
  }
  {
    ...
    rep(i,n) b[i]=p[i][1];
    sort(all(b));
    rep(i,n) if(m_b.count(b[i])==0)m_b[b[i]]=m_b.size();
    rep(i,n){
      p[i][0]*=-1;
      p[i][1]=m_b[p[i][1]]+1;
    }
  ...
  VV<ll> q;
  rep(i,n){
    if(i==0||p[i][0]!=p[i-1][0]||p[i][1]!=p[i-1][1])q.push_back({p[i][1],0LL});
    q.back()[1]++;
  }
  //each(x,q)out(x[0],x[1]);

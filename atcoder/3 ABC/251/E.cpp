...
int main(){
    ...
    V<ll> a(n);
    input(a);
    VV<ll> dp(2,V<ll>(n,INF));
    rep(i,n-1){
        if(i-1>=0)chmin(dp[0][i+1],dp[0][i-1]+a[i]);
        else chmin(dp[0][i+1],a[i]);
        chmin(dp[0][i+1],dp[0][i]+a[i]);
        
        if(i!=0){
            if(i-1>=1)chmin(dp[1][i+1],dp[1][i-1]+a[i]);
            else chmin(dp[1][i+1],a[i]);
            chmin(dp[1][i+1],dp[1][i]+a[i]);
        }
    }
    //debug(dp);
    ll ans=INF;
    chmin(ans,dp[0][n-1]);
    chmin(ans,dp[0][n-2]+a[n-1]);
    chmin(ans,dp[1][n-1]+a[n-1]);
    chmin(ans,dp[1][n-2]+a[n-1]);
    if(n==2)chmin(ans,a[0]);
    if(n==2)chmin(ans,a[1]);
    print(ans);
    return 0;
}
...
int main(){
	...
	REP(i,q){
		vl s_,t_;
		ll rs=binSearch(s,x[i]);
		s_.pb(s[rs]);s_.pb(s[rs+1]);
		ll rt=binSearch(t,x[i]);
		t_.pb(t[rt]);t_.pb(t[rt+1]);
		ll ans=INF;
		REP(j,2)REP(k,2){
			ll tmp=0;
			if(min(s_[j],t_[k])<x[i] && x[i]<max(s_[j],t_[k])){
				tmp+=abs(s_[j]-t_[k]);
				tmp+=min(abs(s_[j]-x[i]),abs(t_[k]-x[i]));
			}else{
				tmp+=max(abs(s_[j]-x[i]),abs(t_[k]-x[i]));
			}
			if(ans>tmp)ans=tmp;
		}
		out(ans);

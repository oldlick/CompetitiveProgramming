...
int main(){
    ...
    rep(i,n)idx[i]=i;
    do{
        V<ll> under_tmp(rem+n-1);
        reps(i,rem,under_tmp.size())under_tmp[i]=1;
        do{
            //debug(idx,under_tmp);
            V<ll> under(1);
            rep(i,under_tmp.size()){
                if(under_tmp[i]==1)under.push_back(0);
                else under.back()++;
            }
            string x;
            rep(i,n){
            ...
            if(x.size()<3||16<x.size())continue;
            if(t_s.count(x)==0)die(x);
        }while(next_permutation(all(under_tmp)));

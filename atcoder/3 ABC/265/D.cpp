...
int main(){
    ...
    rep(x,n+1){
        debug(x);
        auto it1=lower_bound(all(sum),sum[x]+p);
        if(it1==sum.end()||*it1!=sum[x]+p)continue;
        int y=it1-sum.begin();
        debug(x,y);
        auto it2=lower_bound(all(sum),sum[y]+q);
        if(it2==sum.end()||*it2!=sum[y]+q)continue;
        int z=it2-sum.begin();
        debug(x,y,z);
        auto it3=lower_bound(all(sum),sum[z]+r);
        if(it3==sum.end()||*it3!=sum[z]+r)continue;
        int w=it3-sum.begin();
        debug(x,y,z,w);
        die("Yes");
    }

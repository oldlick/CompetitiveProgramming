...
bool hoge(pair<ll,ll> &now,char c){
    auto tmp=now;
    if(c=='U')tmp.first--;
    if(c=='D')tmp.first++;
    if(c=='L')tmp.second--;
    if(c=='R')tmp.second++;
    if(tmp.first<0 || tmp.first>=h)return false;
    if(tmp.second<0 || tmp.second>=w)return false;
    now=tmp;
    return true;
}
...
int main(){
    ...
    while(true){
        debug(now);
        if(done[now.fi][now.se]==1)die("-1");
        done[now.fi][now.se]=1;
        if(hoge(now,g[now.fi][now.se])==false)break;
    }
    print(now.fi+1,now.se+1);
    return 0;
}
...
int main(){
    ...
    each(x,b)each(y,b){
        if(x==y)continue;
        c.insert({x,{y.fi-x.fi,y.se-x.se}});
    }
    debug(c);

    V<mint> fact(n+10);
    fact[0]=1;
    reps(i,1,n+10) fact[i]=fact[i-1]*i;

    map<pair<ll,ll>,VV<ll>> memo;
    ...
    each(x,b){
        ...
        each(y,memo[x]){
            int i=y[0],j=y[1],k=y[2];
            int sum = i+j+k;
            ans -= fact[sum]/fact[i]/fact[j]/fact[k]*((mint)3).pow(n-sum);
            debug(ans,i,j,k);
        }
    }
    each(x,c){
        if(memo.count(x[0])==0)continue;
        if(memo.count(x[1])==0)continue;
        each(y,memo[x[0]]){
            int pre_sum=y[0]+y[1]+y[2];
            each(z,memo[x[1]]){
                int i=z[0],j=z[1],k=z[2];
                int sum = i+j+k;
                if(pre_sum+sum>n)continue;
                ans += fact[sum]/fact[i]/fact[j]/fact[k]*((mint)3).pow(n-sum-pre_sum);
                debug("c",ans,i,j,k);
            }
        }
    }

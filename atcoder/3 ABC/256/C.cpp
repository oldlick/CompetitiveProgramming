...
int main(){
    ...
    VV<ll> a(3,V<ll>(3));
    ll ans=0;
    for(a[0][0]=1;a[0][0]<=30;a[0][0]++)for(a[0][1]=1;a[0][1]<=30;a[0][1]++){
        for(a[1][0]=1;a[1][0]<=30;a[1][0]++)for(a[1][1]=1;a[1][1]<=30;a[1][1]++){
            ...
            //debug(a);
            bool f=true;
            rep(i,3)rep(j,3)if(a[i][j]<=0)f=false;
            rep(i,3){
                ll sum=0;
                rep(j,3)sum+=a[i][j];
                if(sum!=h[i])f=false;

...
static void dft(vector<std::complex>& func, int inverse) {
    int sz = func.size();
    if (sz == 1)return;
    vector<std::complex> veca, vecb;
    rep(i, sz / 2) {
        veca.push_back(func[2 * i]);
        vecb.push_back(func[2 * i + 1]);
    }
    dft(veca, inverse); dft(vecb, inverse);
    std::complex now = 1, zeta = polar(1.0, inverse * 2.0 * acos(-1) / sz);
    rep(i, sz) {

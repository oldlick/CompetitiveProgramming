...
int main(){
    ...
    V<ll> a(n),b(n);
    input(a,b);
    VV<ll> dp(2,V<ll>(2,INF));
    dp[0][0] = a[0];
    dp[1][1] = 0;
    reps(i,1,n){
        VV<ll> dp2(2,V<ll>(2,INF));
        rep(j,2){
            chmin(dp2[j][0],dp[j][0]+a[i]+b[i-1]);
            chmin(dp2[j][0],dp[j][1]+a[i]);
            chmin(dp2[j][1],dp[j][0]);
            chmin(dp2[j][1],dp[j][1]+b[i-1]);
        }
        dp.swap(dp2);
        debug(i,dp);
    }

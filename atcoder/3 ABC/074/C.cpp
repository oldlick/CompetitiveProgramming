...
int main(){
	...
	REP(i,f+1){
		REP(j,4){
			double from=0,to=0;
			if(i+s[j]+w[j]>f)continue;
			if(dp[i].fi+w[j]==0)continue;
			if(dp[i+s[j]+w[j]].fi==0)goto DO;
			from=(double)(dp[i].se+s[j])/(dp[i].fi+w[j]+dp[i].se+s[j]);
			to=(double)(dp[i+s[j]+w[j]].se)/(dp[i+s[j]+w[j]].fi+dp[i+s[j]+w[j]].se);
			if(from>to){
				DO:;
				if(from<=(double)e/(e+100)){
					dp[i+s[j]+w[j]].fi=dp[i].fi+w[j];
					dp[i+s[j]+w[j]].se=dp[i].se+s[j];
				}
			}
		}
	}
	...
	REP(i,f+1){
		//out(dp[i].fi,dp[i].se);
		if(dp[i].fi==0)continue;
		if(ans[0]==0||(double)dp[i].se/dp[i].fi>(double)ans[1]/ans[0]){
			ans[0]=dp[i].fi;
			ans[1]=dp[i].se;
		}
	}
	out(ans[0]+ans[1],ans[1]);
	return 0;
}
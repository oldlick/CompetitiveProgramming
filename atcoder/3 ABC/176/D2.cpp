	...
	while(1){
		...
		while(q.size()!=0){
			...
			REP(i,4){
				ll tmph=nh+dh[i];
				ll tmpw=nw+dw[i];
				if(tmph<0 || tmph>=v.size() || tmpw<0 || tmpw>=v[0].size())continue;
				if(s[tmph][tmpw]=='#')continue;
				if(v[tmph][tmpw]>v[nh][nw]){
					v[tmph][tmpw]=v[nh][nw];
					q.push(mp(tmph,tmpw));
					q2.erase(mp(tmph,tmpw));
				}
			}
			ll dh2[20]={1,1,-1,-1,2,2,2,2,2,1,0,-1,-2,-2,-2,-2,-2,-1,0,1};
			ll dw2[20]={1,-1,1,-1,-2,-1,0,1,2,2,2,2,2,1,0,-1,-2,-2,-2,-2};
			REP(i,20){
				ll tmph=nh+dh2[i];
				ll tmpw=nw+dw2[i];
				if(tmph<0 || tmph>=v.size() || tmpw<0 || tmpw>=v[0].size())continue;
				if(s[tmph][tmpw]=='#')continue;
				if(v[tmph][tmpw]>v[nh][nw]+1){
					q2[mp(tmph,tmpw)]=v[nh][nw]+1;
				}
			}
			cnt++;
		}

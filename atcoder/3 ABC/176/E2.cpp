...
//unordered_map AC済み 実行時間 173ms
int main(){
	init();
	inl(h,w,m);
	vl v(h);
	vl u(w);
	unordered_map<ll,ll> bom;
	REP(i,m){
		inl(a,b);a--;b--;
		v[a]++;
		u[b]++;
	...
	ll maxv=*(max_element(all(v)));
	ll maxu=*(max_element(all(u)));
	vl idxv;
	vl idxu;
	REP(i,h){
		if(v[i]==maxv)idxv.pb(i);
	}
	REP(i,w){
		if(u[i]==maxu)idxu.pb(i);
	}
	EACH(x,idxv)EACH(y,idxu){
		if(bom.find(x*w+y)==bom.end())die(maxv+maxu);
	}
	out(maxv+maxu-1);
	return 0;
}
...
int main(){
  ...
  input(s);
  //debug(s);
  VV<V<ll>> a(n+1,VV<ll>(n+1,V<ll>(7,-INF)));
  VV<V<ll>> b(n+1,VV<ll>(n+1,V<ll>(7,-INF)));
  VV<V<ll>> c(n+1,VV<ll>(n+1,V<ll>(7,-INF)));
  rep(i,n+1)rep(j,n+1){
    a[i][j][0]=0;
    b[i][j][0]=0;
    c[i][j][0]=0;
  ...
  rep(i,n)rep(j,n){
    if(s[i][j]=='#'){
      rep(k,6){
        a[i+1][j+1][k+1]=a[i-1+1][j-0+1][k]+1;
        b[i+1][j+1][k+1]=b[i-0+1][j-1+1][k]+1;
        c[i+1][j+1][k+1]=c[i-1+1][j-1+1][k]+1;
      }
    }else{
      rep(k,6){
        a[i+1][j+1][k+1]=a[i-1+1][j-0+1][k];
        b[i+1][j+1][k+1]=b[i-0+1][j-1+1][k];
        c[i+1][j+1][k+1]=c[i-1+1][j-1+1][k];
      }
    }
  }
  rep(i,n+1)debug(a[i]);
  ...
  rep(i,n+1)debug(c[i]);
  rep(i,n)rep(j,n){
    if(a[i+1][j+1][6]>=4)die("Yes");
    if(b[i+1][j+1][6]>=4)die("Yes");
    if(c[i+1][j+1][6]>=4)die("Yes");
  }
  print("No");
  return 0;
}

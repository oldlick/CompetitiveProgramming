...
V<priority_queue<ll>> memo;
V<ll> x;
void dfs(VV<ll> &g,ll now){
  ...
  each(next,g[now]){
    ...
    dfs(g,next);
    priority_queue<ll> tmp=memo[next];
    while(tmp.size()!=0){
      memo[now].push(tmp.top());
      tmp.pop();
      if(memo[now].size()>21)memo[now].pop();
    }
  }
}
...
int main(){
  ...
  V<ll> v(q),k(q);
  rep(i,q){
    input(v[i],k[i]);v[i]--;k[i]--;
  }
  dfs(g,0);
  /*rep(i,n){
    priority_queue<ll> tmp=memo[i];
    while(tmp.size()!=0){
      debug(i,tmp.top());
      tmp.pop();
    }
  ...
  rep(i,q){
    priority_queue<ll> tmp=memo[v[i]];
    ll K=tmp.size();
    rep(j,K-k[i]-1)tmp.pop();
    print(-tmp.top());
  }
  return 0;
}

...
int main(){
  ...
  V<ll> a(m),b(m);
  rep(i,m){
    input(a[i],b[i]);a[i]--;b[i]--;
  }
  {
    ll sum=0;
    rep(i,n)sum+=d[i];
    if(sum!=(n-1)*2)die(-1);
    rep(i,m)if(d[a[i]]==1 && d[b[i]]==1)die(-1);
  }
  V<pair<ll,ll>> ans;
  V<ll> isUseList(n);
  rep(i,m){
  ...
  for(int i=n-1;i>=2;i--){
    while(list[i].size()!=0){
      ...
      ans.push_back({list[1].back(),list[i].back()});
      list[1].pop_back();
      list[i-1].push_back(list[i].back());
      list[i].pop_back();
    }
  }
  for(int i=0;i<list[2].size()-1;i++){
    ans.push_back({list[2][i],list[2][i+1]});
  }
  VV<ll> list2(n);
  rep(i,m){
    list2[d[a[i]]].push_back(a[i]);
    list2[d[b[i]]].push_back(b[i]);
  }
  ...
  if(list[2].size()==1){
    ...
    con2.push_back(list.front());
    con2.push_back(list.back());
  }
  return 0;
}
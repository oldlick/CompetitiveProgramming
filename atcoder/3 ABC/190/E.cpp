...
V<ll> dijk(ll s,ll n,VV<pair<ll,ll>> &g){
...
ll rec(ll S, ll v) {
    ...
    if (ret != 0) return ret;
    ret = INF;
    rep(u, V_) { chmin(ret, rec(S ^ (1 << v), u) + G[u][v]*(v!=0)); }
    return ret;
}
...
int main(){
  ...
  in1(c);
  rep(i,k)c[i]--;
  VV<ll> ls(k,V<ll>(k,INF));
  rep(i,k){
    V<ll> ret=dijk(c[i],n,g);
    rep(j,k){
      chmin(ls[i][j],ret[c[j]]);
    }
    //outv(ls[i]);
  }
  V_=k;
  ll ans=INF;
  rep(i,k){
    rep(j,k)rep(l,k)G[(j+i)%k][(l+i)%k]=ls[j][l];
    rep(j,200000)rep(l,20)dp[j][l]=0;
    chmin(ans,rec((1<<V_)-1,0));
  }
  if(ans==INF)die(-1);
  out(ans+1);

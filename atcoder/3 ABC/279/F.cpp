...
int main(){
    ...
    rep(i,q){
        ...
        if(t==1){
            inl(x,y);x--;y--;
            if(box2root[y]==-1)continue;
            if(box2root[x]!=-1){
                uf.unite(box2root[x],box2root[y]);
            }
            box2root[x]=uf.root(box2root[y]);
            root2box[uf.root(box2root[y])]=x;
            box2root[y]=-1;
        }else if(t==2){
            inl(x);x--;
            if(box2root[x]!=-1){
                uf.unite(box2root[x],next_ball);
                box2root[x]=uf.root(box2root[x]);
            }else{
                box2root[x]=next_ball;
            }
            next_ball++;
            root2box[uf.root(box2root[x])]=x;
        }else{
            inl(x);x--;
            print(root2box[uf.root(x)]+1);
        }
        debug(root2box,box2root);
    }
    return 0;
}

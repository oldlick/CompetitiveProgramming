...
ll a,b;
ld f(ll x){
    // debug((ld)b*x,a/sqrt((ld)(1+x)));
    return (ld)b*x+(ld)a/sqrt((ld)(1+x));
}
...
int main(){
    ...
    while(abs(high-low)>2){
        debug(high,low);
        long long d1=low+(high-low)/3;
        long long d2=high-(high-low)/3;
        if(f(d1)<f(d2))high=d2;
        else low=d1;
    }
    debug(high,low);
    V<ld> ans={f(high),f(low),f((high+low)/2)};
    debug(ans);
    sort(all(ans));
    print(ans[0]);
    return 0;

...
int main(){
    init();
    inl(n,m);
    V<pair<ll,ll>> a(n),b(m);
    rep(i,n)input(a[i].fi);
    rep(i,n)input(a[i].se);
    rep(i,m)input(b[i].fi);
    rep(i,m)input(b[i].se);
    sort(all(a));
    reverse(all(a));
    sort(all(b));
    debug(a,b);
    multiset<ll> s;
    each(x,a){
        while(b.size()!=0 && b.back().fi>=x.fi){
            s.insert(b.back().se);
            b.pop_back();
        }
        //debug("s");
        //each(x,s) debug(x);
        auto it=s.lower_bound(x.se);
        if(it==s.end())die("No");
        s.erase(it);
    }
    print("Yes");

...
int main(){
  ...
  UNIQUE(b);
  V<ll> t(n);
  rep(i,n)t[i]=lower_bound(all(b),a[i])-b.begin();

  ll m=b.size();
  LazySegmentTree seg(vector<mint>(m+1,0));

  seg.add(0,1,1);
  seg.add(0,2,((mint)1)/2);
  out(seg.getsum(0,1)*2);
  out(seg.getsum(1,2)*2);
  out(seg.getsum(0,2)*2);

  mint ans=0;
  for(int i=0; i<n; i++) {
    if(i!=0){
      out("sum",seg.getsum(0, t[i]+1)* ((mint)2).pow(i-1));
      ans+=seg.getsum(0, t[i]+1) * ((mint)2).pow(i-1);
    }
    seg.add(t[i], t[i]+1, (mint)1/((mint)2).pow(i));
    //out(ans);
  }
  out(ans);

...
ll conv(string s){
    ...
    if(s.size()==2) return (s[1]-'0')*26+(s[0]-'0')+26;
    if(s.size()==3) return (s[2]-'0')*26*26+(s[1]-'0')*26+(s[0]-'0')+26*26;
    return 0;
}
...
int main(){
    ...
    rep(i,26)rep(j,26)rep(k,26){
        ...
        s.push_back('a'+j);
        s.push_back('a'+k);
        rep(l1,3)reps(l2,1,3-l1+1){
            g2[s]+=g[s.substr(l1,l2)];
        }
    }
    rep(i,26)rep(j,26){
        string s;

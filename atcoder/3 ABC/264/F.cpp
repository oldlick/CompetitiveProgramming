...
int main(){
    ...
    input(r);
    input(c);
    V<ll> sum_r=r,sum_c=c;
    reps(i,1,h)sum_r[i]+=sum_r[i-1];
    reps(i,1,w)sum_c[i]+=sum_c[i-1];
    VV<ll> a(h,V<ll>(w));
    rep(i,h){
        ins(s);
        rep(j,w)a[i][j]=s[j]-'0';
    ...
    VV<VV<ll>> dp(h,VV<V<ll>>(w,VV<ll>(2,V<ll>(2,INF))));
    dp[0][0][0][0]=0;
    rep(i,h)rep(j,w)rep(k,2)rep(l,2){
        if(i+1<h){
            if(a[i+1][j]==(a[i][j]^k)){
                chmin(dp[i+1][j][0][l],dp[i][j][k][l]);
            }else{
                chmin(dp[i+1][j][0][l],dp[i][j][k][l]+sum_r[i]);
                chmin(dp[i+1][j][1][l],dp[i][j][k][l]+r[i+1]);
            }
        }
        if(j+1<w){
            if(a[i][j+1]==(a[i][j]^l)){
                chmin(dp[i][j+1][k][0],dp[i][j][k][l]);
            }else{
                chmin(dp[i][j+1][k][0],dp[i][j][k][l]+sum_c[j]);
                chmin(dp[i][j+1][k][1],dp[i][j][k][l]+c[j+1]);
            }
        }
        //debug(dp);
    }

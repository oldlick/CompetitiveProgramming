...
template<class T>
tuple<V<T>,V<ll>> dijk(VV<pair<ll,T>> &g,V<ll> &s){
  ll n=g.size();
  priority_queue<pair<T,ll>,vector<pair<T,ll>>,greater<pair<T,ll>>> q;
  each(x,s)q.push(make_pair(0,x));
  vector<ll> pre(n,-1);
  V<ll> cost(n,-INF);
  vector<ll> done(n);
  each(x,s)cost[x]=0;
...
int main(){
    ...
    debug(l);
    V<ll> s;
    rep(i,n)if(uf.root(i)==i)s.push_back(i);
    auto [cost,pre] = dijk(g,s);
    debug(cost);
    V<ll> loop(n);
    each(x,l){
        if(cost[x[1]]<cost[x[0]]+x[2]){
            loop[uf.root(x[0])]=1;
        }
    }
    debug(loop);

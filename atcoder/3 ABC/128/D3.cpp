...
int main(){
  ...
  V<ll> subL(n+1),subR(n+1);
  rep(i,n)subL[i+1]=subL[i]+(v[i]<0?v[i]:0);
  rep(i,n)subR[i+1]=subR[i]+(v[n-1-i]<0?v[n-1-i]:0);
  
  ll ans=0;
  rep(i,n)rep(j,n-i){
    ll rem=k-i-j;
    if(rem<0)continue;
    ll tmp=sumL[i]+sumR[j];
    chmax(ans,tmp);
    rep(k,rem+1){
      out(tmp-subL[min(k,i)]-subR[min(rem-k,j)],i,j,k);
      chmax(ans,tmp-subL[min(k,i)]-subR[min(rem-k,j)]);
    }
  }
  out(ans);
  return 0;

...
void dfs(V<string> &c,ll nh,ll nw,ll mark){
    debug(nh,nw);
    c[nh][nw]='0'+mark;
    int dh[4]={0,0,1,-1};
    int dw[4]={1,-1,0,0};
    rep(i,4){
        ll th=nh+dh[i];
        ll tw=nw+dw[i];
        if(ch(th,tw)==false)continue;
        if(c[th][tw]!='.')continue;
        dfs(c,th,tw,mark);
    }
}

void exe(V<string> &c,ll sh,ll sw,ll mark){
    if(ch(sh,sw)==false) return;
    if(c[sh][sw]=='.'){
        dfs(c,sh,sw,mark);
        return;
    }
    if(0<=c[sh][sw]-'0'&&c[sh][sw]-'0'<4){
        die("Yes");
    }
}

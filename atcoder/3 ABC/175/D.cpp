...
int main(){
	...
	while(1){
		bool change=false;
		REP(i,n){
			if(data[i].size()!=0 && seq[i]==i)continue;
			seq[i]=p[seq[i]];
			if(data[i].size()==0) data[i].pb(c[seq[i]]);
			else data[i].pb(data[i].back()+c[seq[i]]);
			change=true;
		}
	...
	REP(i,n){
		...
		//outv(data[i]);
		if(data[i].back()>0){
			ll loop=k/data[i].size(), rem=k%data[i].size();
			REP(j,data[i].size()){
				if(j<rem){
					if(now<data[i].back()*loop+data[i][j])now=data[i].back()*loop+data[i][j];
				}else{
					if(now<data[i].back()*(loop-1)+data[i][j])now=data[i].back()*(loop-1)+data[i][j];
				}
			}
		}else{
			REP(j,data[i].size()){
				if(now<data[i][j])now=data[i][j];
			}
		}
		if(ans<now)ans=now;
	}

...
mint hoge(int r,int g,int b,int k){
    int r_=r-k;
    int g_=g-k;
    int b_=b;
    int sum=r_+g_+b_+k;
    mint ret=COM.fact[sum]/COM.fact[r_]/COM.fact[g_]/COM.fact[b_]/COM.fact[k];
    debug(r,g,b,k,ret);
    return ret;
}
...
int main(){
    ...
    int min_rg=min(r,g);
    V<mint> ans(min_rg+1);
    for(int k_=min_rg;k_>=k;k_--){
        if(k_==min_rg){
            ans[k_]=hoge(r,g,b,k_);
        }else{
            ans[k_]=hoge(r,g,b,k_)-ans[k_+1]*k_-hoge(r,g,b,k_+1);
        }
        debug
    }
    debug(ans);

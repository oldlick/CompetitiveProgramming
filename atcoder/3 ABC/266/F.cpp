...
V<ll> loop;
bool hoge(VV<ll> &g,ll now,ll pre=-1){
    static V<ll> done(g.size());
    done[now]=1;
    debug(now);
    each(x,g[now]){
        if(pre==x)continue;
...
int main(){
    ...
    debug(e);
    rep(i,loop.size()){
        debug(loop[(i+1)%loop.size()],loop[i]);
        e.erase({loop[(i+1)%loop.size()],loop[i]});
        e.erase({loop[i],loop[(i+1)%loop.size()]});
    }

...
void dfs(vl num,vl usef,const vl &dis,ll underf){
	//outv(num);
	if(num.size()==dis.size()){
		if(usef[3]>0 && usef[5]>0 && usef[7]>0){
			ans++;
			//outv(num);
			//out("find");
		}
	...
	REP(i,4){
		...
		nnum.pb(add[i]);
		nusef[add[i]]++;
		if(add[i]<dis[nnum.size()-1]){
			nunderf=true;
		}
		dfs(nnum,nusef,dis,nunderf);
	}
}

...
void dfs(vl &div,ll k,ll &ans,vl &cnt){
	div.pb(k);
	double calc=1,seq=1;
	REREP(i,div.size()){
		if(i+1<div.size() && div[i]==div[i+1])seq++;
		else seq=1;
		calc*=cnt[div[i]]-(div.size()-1-i);
		calc/=seq;
	}
	//outv(div);out(ans,"+=",(int)calc);
	ans+=max((int)calc,0);
	div.pop_back();
	ll start=2;
	if(div.size()>0)start=div.back();
	for(ll i=start;i*i<=k;i++){
		if(k%i==0){
			div.pb(i);
			dfs(div,k/i,ans,cnt);
			div.pop_back();

...
template <typename T> void out1(T s) {REP(i,s.size()){in(s[i]);}}
template <typename T> void out2(T s,T t) {REP(i,s.size()){in(s[i],t[i]);}}
template <typename T> void out3(T s,T t,T u) {REP(i,s.size()){in(s[i],t[i],u[i]);}}
template <typename T> void out4(T s,T t,T u,T v) {REP(i,s.size()){in(s[i],t[i],u[i],v[i]);}}
template <typename T> void out5(T s,T t,T u,T v,T w) {REP(i,s.size()){in(s[i],t[i],u[i],v[i],w[i]);}}
...
int main(){
	...
	while(q.size()!=0){
		...
		REP(i,4){
			ll nextx=v.fi+xd[i],nexty=v.se+yd[i];
			if(nextx<0||nexty<0||nextx>=h||nexty>=w)continue;
			if(s[nextx][nexty]=='.' && ans[nextx][nexty]==0){
				ans[nextx][nexty]=ans[v.fi][v.se]+1;
				q.push(make_pair(nextx,nexty));
			}
		}
	}
	if(ans[h-1][w-1]==0)die(-1);
	out(cntw-ans[h-1][w-1]);
	return 0;
}
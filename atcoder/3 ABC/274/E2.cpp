...
int main(){
    ...
    rep(i,v)rep(j,v){
        ll dx=px[i]-px[j];
        ll dy=py[i]-py[j];
        g[i].push_back({j,sqrt(dx*dx+dy*dy)});
    }
    //debug(g);
    VV<pair<ld,ld>> dp(1<<v,V<pair<ld,ld>>(v,{INF,1}));
    dp[0][0] = {0,1};
    rep(S, 1<<v){
        rep(a,v)each(x,g[a]){
            ll b=x.fi;
            ld d=x.se;
            if(b==a) continue;
            if (S != 0 && !(S & (1<<a))) continue; //aは含め
            if ((S & (1 << b)))continue; //bは含むな
            auto nx = dp[S | (1 << b)][b];
            auto now = dp[S][a];
            auto pre = now;
            pre.fi += d*now.se;
    ...
    rep(i,1<<m){
        //debug((1<<(n+1))-1,(i<<(n+1)));
        ld tmp=(double)dp[(1<<(n+1))-1+(i<<(n+1))][0].fi;
        //debug(dp[(1<<(n+1))-1+(i<<(n+1))][0]);
        chmin(ans,tmp);
    }
    print(ans);
    return 0;

...
bool hoge(V<ll> a, ll v){
    V<ll> dp(20010);
    dp[10000+a[0]]=1;
    reps(i,1,a.size()){
        ll x=a[i];
        V<ll> dp2(20010);
        rep(i,dp.size())if(dp[i]==1){
            if(i+x<dp.size())dp2[i+x]=1;
            if(i-x>=0)dp2[i-x]=1;
        }
        dp2.swap(dp);
    }
    //debug("--");
    //rep(i,dp.size())if(dp[i]==1)debug(i);
    return dp[v]==1;
}

int main(){
    init();
    inl(n,x,y);x+=10000;y+=10000;
    debug(x,y);
    V<ll> a,b;
    b.push_back(0);
    rep(i,n){
    ...
    debug(a,b);
    if(hoge(a,x)&&hoge(b,y))print("Yes");
    else print("No");
    return 0;
}

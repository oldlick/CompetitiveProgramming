...
ll hoge(V<ll> &g_w,V<ll> &g_a,V<ll> &g_b,ll range,ll idx){
    V<ll> a,b,w;
    rep(i,g_a.size()) if(i!=idx) a.push_back(g_a[i]-g_a[idx]);
    rep(i,g_b.size()) if(i!=idx) b.push_back(g_b[i]-g_b[idx]);
    rep(i,g_w.size()) if(i!=idx) w.push_back(g_w[i]);
    debug(a,b,w);
    ll n=a.size();
    V<pair<frac,ll>> m;
    ll base=g_w[idx];
    rep(i,n){
        if(a[i]==0){
            if(0<=b[i]&&b[i]<=range) base+=w[i];
        }else{
            frac t_begin = frac(-b[i],a[i]);
            frac t_end = frac(range-b[i],a[i]);
            if(t_begin>t_end) swap(t_begin,t_end);
            m.push_back({t_begin,(w[i])*(-1)});
            m.push_back({t_end,(-w[i])*(-1)});
        }
    }
    sort(all(m));
    debug(m);
...
int main(){
    ...
    ll mx=0;
    rep(i,n) chmax(mx,hoge(w,v,x,a,i));
    print(mx);
    return 0;
}

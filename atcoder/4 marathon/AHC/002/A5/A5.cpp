...
tuple<ll,V<ll>,V<ll>> solve(VV<V<ll>> &priority){
  ll nwX=a;
  ll nwY=b;
  ll nwV=0;
  V<ll> nwA;
...
ll changeCnt=5304009;
tuple<ll,V<ll>,V<ll>,bool> change(ll &gtV,V<ll> &gtA,V<ll> &gtL,VV<V<ll>> &priority){
  ll nwV=gtV;
  V<ll> nwA;
  V<ll> nwL=gtL;
  ll nwX=0;
  ...
  ll edX=a;
  ll edY=b;
  std::uniform_int_distribution<> range2(0,min((int)10,(int)gtA.size()));
  ll rand2=range2(mt);
  ...
  if(mxV!=-1){
    ...
    return {mxV,mxA,mxL,changeCnt<0};
  }
  return {gtV,gtA,gtL,changeCnt<0};
}
...
int main(){
  ...
  rep(rev,2){
    rep(start,4){
      ...
      auto [nwV,nwA,nwL]=solve(priority);
      if(chmax(mxV,nwV)){
        //outv(path);
        mxA=nwA;
        mxL=nwL;
  ...
  while(true){
    auto [nwV,nwA,nwL,endFlag]=change(mxV,mxA,mxL,priority);
    if(endFlag)break;
    if(chmax(mxV,nwV)){
      mxA=nwA;
      mxL=nwL;

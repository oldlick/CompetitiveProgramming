...
ld variance(V<FractionC> a,ll s,ll t){
  ...
  ld cnt=0;
  reps(i,s,t){
    sum+=a[i].weight*a[i].trust;
    cnt+=a[i].trust;
  }
  if(cnt<0.0001)return 0;
  reps(i,s,t){
    sum_2+=powl(a[i].weight-sum/cnt,2)*a[i].trust;
  }
  return powl(sum_2/cnt,0.5);
}
...
int main(){
      ...
      rep(mode,2){
        ...
        rep(i,30){
          rep(j,29){
            ...
            /*if(nowPoint.weight<nowPoint.estimated-randomDValue){
              ll value=0;
              value+=nowPoint.weight*nowPoint.trust;
              value+=(nowPoint.estimated-randomDValue)*randomDTrust;
              set(value/(nowPoint.trust+randomDTrust));
            }else if(nowPoint.weight>nowPoint.estimated+randomDValue){
              ll value=0;
              value+=nowPoint.weight*nowPoint.trust;
              value+=(nowPoint.estimated-randomDValue)*randomDTrust;
              set(value/(nowPoint.trust+randomDTrust));
            }*/
            ld range=randomDValue*(4/randomDTrust+1);
...
#endif
    {//実測値記録
      ld difference=b-cost[t];
      ld trustInvSum=0;
      rep(i,ans.size()-1)trustInvSum+=1/(g[ansPlace[i]][ansPlace[i+1]].trust+1);
      V<ll> length(ans.size(),1);
      rep(i,length.size()-1){
        if(ans[i+1]==ans[i])length[i+1]=length[i]+1;
        else length[i+1]=1;
      }
      for(ll i=length.size()-1,now=length.back();i>=0;i--){
        ll next=now;
        if(length[i]==1&&i!=0)next=length[i-1];
        length[i]=now;
        now=next;
      }
      rep(i,ans.size()){
        ld addWeight=difference*(1/trustInvSum/(g[ansPlace[i]][ansPlace[i+1]].trust+1));
        ld addTrust=1;
        addTrust*=sigmoid(addWeight,1000,3000,1.3,0.7);
        addTrust*=sigmoid((ld)length[i]/ansPlace.size(),0.2,0.8,0.5,1.5);
        //out(ansPlace[i]/30,ansPlace[i]%30,ansPlace[i+1]/30,ansPlace[i+1]%30,addTrust);
        g[ansPlace[i]][ansPlace[i+1]].weight+=addWeight;
        g[ansPlace[i+1]][ansPlace[i]].weight+=addWeight;
        chmax(g[ansPlace[i]][ansPlace[i+1]].weight,0.1);
        chmax(g[ansPlace[i+1]][ansPlace[i]].weight,0.1);
        g[ansPlace[i]][ansPlace[i+1]].trust+=addTrust;
        g[ansPlace[i+1]][ansPlace[i]].trust+=addTrust;
      }
    }//実測値記録終わり
  }
#ifndef ONLINE_JUDGE
  {
    ...
    auto output=[&](ll p1,ll p2){
        ...
        if(g[p1][p2].trust==0)cerr<<" -";
        else cerr<<setw(2)<<(int)g[p1][p2].trust;
        cerr<<"]";

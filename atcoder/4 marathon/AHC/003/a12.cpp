...
int main(int argc,char* argv[]){
  ...
  V<tuple<bitset<EDGE_NUM>,ll,ld>> edgesBitSet;
#ifndef ONLINE_JUDGE
  ll parameter;
  if(argc<=1){
    parameter=1;
  ...
  for(loop=0;loop<1000;loop++){
    ...
    in(b);
#endif
    auto weightWrite=[&g](ll cost,bitset<EDGE_NUM> edgesBit,ld err){//実測値記録
      ld alpha=sigmoid(err/cost,0.5,0.3,0,1);
      V<ll> edges;
    ...
    weightWrite(b,edgesBit,b*0.1111);
    
    ll end=edgesBitSet.size();
    rep(i,end){
      auto x=edgesBitSet[i];
      auto orBit=get<0>(x)|edgesBit;
      if(orBit!=get<0>(x) && orBit!=edgesBit)continue;
      auto xorBit=get<0>(x)^edgesBit;
      if(!xorBit.any())continue;
      weightWrite(abs(b-get<1>(x)),xorBit,b*0.1+get<2>(x));
      edgesBitSet.push_back({xorBit,abs(b-get<1>(x)),b*0.1+get<2>(x)});
#ifndef ONLINE_JUDGE
      edgesBitSetCnt[loop]++;
#endif
    }
    edgesBitSet.push_back({edgesBit,b,b*0.1});
    if(loop%230==0 && loop>0){
      ...
      rep(i,edgesBitSet.size()){
        auto x=edgesBitSet[i];
        weightWrite(get<1>(x),get<0>(x),get<2>(x));
      }
    }
  }
#ifndef ONLINE_JUDGE

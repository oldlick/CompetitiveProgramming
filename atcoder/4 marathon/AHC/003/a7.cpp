...
class FractionC{
  ...
  ld z() const {//推定値
    if(estimated<=0){
      cerr<<"cost error["<<estimated<<"]"<<endl;
      exit(0);
    }
    return estimated;
  }
  ...
  for(loop=0;loop<1000;loop++){
      ...
      rep(mode,2){
        ...
        rep(i,30){
          ...
          rep(j,29){
            ...
            if(j<=divPoint){
              set((leftAvg+g[x*30+y][x*30+y+d].weight)/2);
            }else{
              set((rightAvg+g[x*30+y][x*30+y+d].weight)/2);
            }
          }
        }
      }
    ...
    {//実測値記録
      ...
      rep(i,ans.size()){
        ...
        chmax(g[ansPlace[i]][ansPlace[i+1]].weight,1);
        chmax(g[ansPlace[i+1]][ansPlace[i]].weight,1);

...
int main(){
  ...
  for(loop=0;loop<1000;loop++){
      ...
      rep(mode,2){
        ...
        rep(i,30){
          ...
          rep(j,29){
            ...
            if(nowPoint.trust<=2 && j==divPoint+1){
              divPoint++;
              ld mn=min(leftAvg,rightAvg);
              randomDValue+=abs(nowPoint.weight-mn)*nowPoint.trust;
    ...
    in(b);
#endif
    auto weightWrite=[&g](ld cost,bitset<EDGE_NUM> edgesBit){//実測値記録
      V<ll> edges;

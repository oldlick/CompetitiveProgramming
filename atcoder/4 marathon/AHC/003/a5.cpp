...
int main(){
...
#endif
    {
      ...
      rep(i,ans.size()-1){
        ...
        if(x==0){
          ...
          reps(y,0,30){
            ll p=x*30+y;
            each(j,g[p])if(abs(j.fi-p)==1){
              ll d=abs(y-ans[i]%30);
              ld alpha=1;    //縦横全て
              alpha=1/sqrt(d*loop+1);    //伝播
              //alpha=(d==0?1:0);    //pathのみ
              j.se.x+=(double)cost*alpha;
              j.se.y+=(double)alpha;
            }
          }
        }else{
...
#ifndef ONLINE_JUDGE
  {
    ...
    rep(x,30)rep(y,30){
      ll p=x*30+y;
      if(y==0)cerr<<endl;
      rep(j,g[p].size())if(g[p][j].first/30==x){
        if((int)g[p][j].se.z()==5000)cerr<<"_____";
        else cerr<<setw(5)<<(int)g[p][j].se.z();
        cerr<<"[";
        if((int)g[p][j].se.z()==5000)cerr<<"_____";
        else cerr<<setw(5)<<(int)g[p][j].se.y;
        cerr<<"]";
        cerr<<"("<<setw(4)<<p<<","<<setw(4)<<g[p][j].fi<<") ";
      }
    }
    cerr<<endl;
    rep(x,30)rep(y,30){
      ...
      if(y==0)cerr<<endl;
      rep(j,g[p].size())if(g[p][j].first/30==x){
        cerr<<setw(5)<<(int)(g[p][j].se.z()-gc[p][j].se)<<" ";
      }
    }
    rep(y,30)rep(x,30){
      ll p=x*30+y;
      if(x==0)cerr<<endl;
      rep(j,g[p].size())if(g[p][j].first%30==y){
        if((int)g[p][j].se.z()==5000)cerr<<"_____";
        else cerr<<setw(5)<<(int)g[p][j].se.z();
        cerr<<"[";
        if((int)g[p][j].se.z()==5000)cerr<<"_____";

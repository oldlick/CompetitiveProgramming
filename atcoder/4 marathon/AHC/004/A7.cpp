...
void fastCalc(draft &ans,ll x,ll y,ll length,ll dir){
  ...
  each(it,ans.lines){
    ...
    if(f)lines.push_back(it);
      ...
      rep(l,s[it.idx].size()){
        ans.use[(it.x+l*it.dir)%n][(it.y+l*(1-it.dir))%n]--;
      }
      //out("sub:idx=",it.idx);
    }
  }
...
//焼きなまし法 高速化 デバッグ ビームサーチ幅増やす 良さそうなところを変更していく
int main(){
  ...
  std::uniform_int_distribution<> rand100(0, 99);
  {
    ...
    while(1){
      ...
      V<pair<ll,ll>> lowUsePlace;
      rep(i,n)rep(j,n){
        if(now.use[i][j]==0){
          lowUsePlace.push_back({i,j});
          lowUsePlace.push_back({i,j});
          lowUsePlace.push_back({i,j});
          lowUsePlace.push_back({i,j});
        }
        ...
        if(now.use[(i+1)%n][j]==0){
          lowUsePlace.push_back({i,j});
        }
        if(now.use[i][(j+1)%n]==0){
          lowUsePlace.push_back({i,j});
        }
      }
      ll dir=rand2(mt);
      ...
      if(lowUsePlace.size()!=0){
        std::uniform_int_distribution<> randP(0, lowUsePlace.size()-1);
        auto tmp=lowUsePlace[randP(mt)];
        x=tmp.fi;
        y=tmp.se;
      }
      if(now.use[x][(y+1)%n]<now.use[(x+1)%n][y]){
        dir=0;
      }else if(now.use[x][(y+1)%n]>now.use[(x+1)%n][y]){
        dir=1;
      }else if(now.use[x][(y+2)%n]<now.use[(x+2)%n][y]){
        dir=0;
      }else if(now.use[x][(y+2)%n]>now.use[(x+2)%n][y]){
        dir=1;
      }else if(now.use[x][(y+3)%n]<now.use[(x+3)%n][y]){
        dir=0;
      }else if(now.use[x][(y+3)%n]>now.use[(x+3)%n][y]){
        dir=1;
      }else if(now.use[x][(y+4)%n]<now.use[(x+4)%n][y]){
        dir=0;
      }else if(now.use[x][(y+4)%n]>now.use[(x+4)%n][y]){
        dir=1;
      }else if(now.use[x][(y+5)%n]<now.use[(x+5)%n][y]){
        dir=0;
      }else if(now.use[x][(y+5)%n]>now.use[(x+5)%n][y]){
        dir=1;
      }else if(now.use[x][(y+6)%n]<now.use[(x+6)%n][y]){
        dir=0;
      }else if(now.use[x][(y+6)%n]>now.use[(x+6)%n][y]){
        dir=1;
      }else if(now.use[x][(y+7)%n]<now.use[(x+7)%n][y]){
        dir=0;
      }else if(now.use[x][(y+7)%n]>now.use[(x+7)%n][y]){
        dir=1;
      }

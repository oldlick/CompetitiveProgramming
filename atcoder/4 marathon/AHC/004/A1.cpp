...
ll calcScore(V<string> &ans){
  ll cnt=0;
  rep(k,m){
    rep(dir,2)rep(i,n)rep(j,n){
      bool f=true;
      rep(l,s[k].size()){
        if(s[k][l]!=ans[(i+l*dir)%n][(j+l*(1-dir))%n]){
          f=false;
          break;
        }
      }
...
int main(){
  ...
  V<string> ans(n);
  {
    ll cnt=0;
    rep(i,n){
      while(ans[i].size()+s[cnt].size()<=n)ans[i]+=s[cnt++];
      ans[i]+=string(n-ans[i].size(),'.');
    }
  }
  ...
  {
    V<pair<V<string>,ll>> list(2);
    list[0]={ans,calcScore(ans)};
    list[1]={ans,calcScore(ans)};
    ll cnt=0;
    while(1){
      ...
      auto now=list[cnt%5==0].fi;
      rep(l,s[writeLine].size()){
        now[(x+l*dir)%n][(y+l*(1-dir))%n]=s[writeLine][l];
      }
      ll score=calcScore(now);
      if(list[0].se<score)list[0]={now,score};
      else if(rand100(mt)<20)list[1]={now,score};
      cnt++;
    }
    //cerr<<cnt<<endl;
    ans=list[0].fi;
  }
  cerr<<(ll)(calcScore(ans)*1e8/m)<<endl;
  rep(i,n){
    out(ans[i]);
  }
  return 0;

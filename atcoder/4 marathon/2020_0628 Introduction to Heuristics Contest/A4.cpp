	...
	REP(k,10){
		REP(i,d){
			...
			REP(j,26){
				...
				vl t2=t;
				t2[date]=j;
				ll tmp=calc(d,c,s,t2);
				if(tmp>max)change=j;
			}
			if(change!=-1)t[date]=change;
		}
	}
	vvl top5(d,vl (5));
	REP(i,d){
